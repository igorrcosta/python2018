...
class Game(Tk):
    ...
    def draw_X(self, grid_x, grid_y):
        ...
        img_x = PhotoImage(file=X_FILE_NAME)
        label = Label(image=img_x)
        label.image = img_x
    
        self.canvas.create_image(x, y, image = img_x)
##
##        self.canvas.create_line(
##            x-delta, y-delta,
##            x+delta, y+delta,
...
##            width=SYMBOL_WIDTH, fill=X_COLOR)
    ...
    def draw_O(self, grid_x, grid_y):
        ...
        y = self.gtpix(grid_y)
        delta = CELL_SIZE/2*SYMBOL_SIZE
        img_o = PhotoImage(file=O_FILE_NAME, height = 100, width = 100)
        label = Label(image=img_o)
        label.image = img_o
        self.canvas.create_image(x, y, image = img_o)
...
#x and o images
X_FILE_NAME = filedialog.askopenfilename()
O_FILE_NAME = filedialog.askopenfilename()
